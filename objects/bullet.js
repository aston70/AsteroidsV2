class Bullet {
  ...
  // Default to grey.
  constructor(objectFired, pos, angle, color = [169, 169, 169]) {
    this.objectFired = objectFired;
    this.pos = createVector(pos.x, pos.y);
    let offset = p5.Vector.fromAngle(angle - HALF_PI).mult(20);
    this.pos.add(offset);
    this.vel = p5.Vector.fromAngle(angle - HALF_PI).mult(7);
  ...
  hits(target) {
    ...
    switch (target.constructor.name) {
      ...
      case 'UFO':
        ...
        let ufoD = dist(this.pos.x, this.pos.y, target.pos.x, target.pos.y);
  ...
  // Set a target for the bullet to track.
  setTarget(target) {
    this.target = target;
  }

  // Check if the bullet missed the asteroid.
  checkIfMissed() {
    if (this.target) {
      ...
      // Miss if the distance is greater than twice the asteroid's radius.
      return distanceToTarget > this.target.r * 2;  
    }
    return false;
  }
